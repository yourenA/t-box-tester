...
let CANID_FRAME_HOST = 0x74D;
let CANID_FRAME_ECU  = 0x7CD;
let CANID_DRAWER_HOST = 0x74D;
let CANID_DRAWER_ECU  = 0x7CD;
...
function OpenCanDevice(setting, errCb) {
    ...
    if (0 != device.connect(setting.protocol, setting.baudrate, setting.flags)) {
        device.close();
        errCb("Bus connect failure.");
        return -1;
    }

    frameFilter = device.startMsgFilter(setting.filterType, setting.mask, setting.CANID_FRAME_ECU, setting.CANID_FRAME_HOST);
    ...
    drawerFilter =  device.startMsgFilter(setting.filterType, setting.mask, setting.CANID_DRAWER_ECU, setting.CANID_DRAWER_HOST);
...
/*
 * 设置DUT电源（启动关闭测试）
 * flags, 8位长度数值类型，每位代表对应的DUT电源开关，0为关，1为开
 * limit, 限制电流，单位毫安。超出该值时，自动关闭电源，范围：0 - 50000
 */
function SetupDutPower(setting,errCb) {
    var request = Buffer.from([setting.SID_WRITE_DATA_BY_IDENTIFIER,
        setting.DRAWER_DID_DUT_SWITCH >> 8,
        setting.DRAWER_DID_DUT_SWITCH,
        setting.limit >> 8,
        setting.limit,
        setting.flags]);
    /* 发送设置请求 */
    var ret = device.send(setting.CANID_DRAWER_HOST, request, 1000);
    if (0 != ret) {
...
function GetDutInfo(index,setting,cb,errCb) {
    var request = Buffer.from([setting.SID_READ_DATA_BY_IDENTIFIER, 0x01, 0x10 + index]);
    /* 发送设置请求 */
    var ret = device.send(setting.CANID_DRAWER_HOST, request, 1000);
    if (0 != ret) {
        console.log("Send request failure %d.", ret);
